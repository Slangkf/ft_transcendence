...
import { MatchPlayer } from "src/g/types";
...
export class MultiPlayerFacade {
    ...
    async handleAllReady(roomId: string): Promise<void>{
        const room = await this.roomService.getRoom(roomId);
        if (!room || room.status !== "waiting") return;
        await this.roomService.updateStatus(room, 'starting');

        try{
            await this.multiService.startGame(room); 

            //session update game status ingame, and gameid 
            for(const p of Object.values(room.players)){
                await this.sessionService.update(p.id, {
                    status: "in_game",
                    gameId: room.sessionId,
                })
            }
            //notify all players 
    ...
    async joinMatchmaking(mode: string, userId: string, nickname: string): Promise<{status: 'matched'|'waiting'; players?:MatchPlayer[]; roomId?: string}>{
    ...
    async trymatch(mode: string): Promise<{status: 'matched'|'waiting'; players?:MatchPlayer[]; roomId?: string}>{
        ...
        for(const player of match.players){
            //update session information, roomid ans matchid 
    ...
    async setPlayerReady(roomId: string, userId: string, isReady: boolean): Promise<void> {
        ...
        await this.handleAllReady(roomId);
    }
