import { IGameRepository, SoloGameState, Player } from "src/g/types";
import { GameMode, GameUpdateResponse } from "./game.types";
...
export class SoloService extends GameBaseService{
    ...
    async submitAnswer(gameId: string, selectedAnswerIndex: number, userId: string): Promise<GameUpdateResponse>{
        ...
        const {isCorrect, correctIndex, correctText} = this.processAnswer(state, selectedAnswerIndex, userId);
        const allAnswered = Object.values(state.players)
            .filter(p => !p.isAI || state.mode === GameMode.AI)
